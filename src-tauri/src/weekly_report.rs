...
fn summary_prompt(facts: &str) -> String {
    format!(
        "你是资深研发周报助手。请**仅依据**下面给出的本周事实证据（agent 会话 + git 提交）综合分析，回答「本周到底做了什么」。\n\
         输出必须严格遵循以下固定模板，三个章节的标题文字与顺序不得改动，不得增删章节：\n\
         \n\
         ## 本周概览\n\
         （1-2 句，概括本周总量与主要方向）\n\
         \n\
         ...
         \n\
         ## 问题与卡点\n\
         （有明显迹象时写具体内容，否则整节只写「本周无明显卡点」）\n\
         \n\
         具体要求：\n\
         - 仅依据事实证据归纳，突出交付与影响，不罗列原始会话或提交流水账，不编造证据中不存在的内容；\n\
         - 「项目进展」下每个项目一个小节，标题为项目名，每节 1-4 条要点，同类提交可合并；只有会话记录、没有提交的项目也照常输出；\n\
         - 除标题外正文控制在 300-400 字；\n\
