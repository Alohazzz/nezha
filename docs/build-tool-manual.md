...
- 面板「仓库拉取」列出自动推导的仓库：主仓库 + `.gitmodules` 子模块。子模块默认只展示 DrugInOut / Term / Hsp.Win，可在**项目设置 → 构建 → 可选子仓库**的多选下拉里勾选要展示的子仓库（不勾选任何一项表示列出全部子模块）。同一份列表也是任务面板顶部**仓库切换器**的子仓库来源：切到某个子仓库后，右侧「**Git 变更**」「**Git 历史**」面板展示的就是该子仓库的记录。
...
- 勾选要拉取的仓库 → 点「**拉取**」：
  ...
  - 结果显示为压缩摘要（`✓/× 仓库: 摘要`）。
- 「**清理失效分支**」删除**远端已不存在的本地分支**（勾选的仓库）：先 `git fetch --prune origin` 刷新远端引用，再逐个判定，先 dry-run 扫描候选、弹确认框列出将删除的分支，确认后才真删。
  - 只删**提交已存在于某个 `origin/*` 分支**的本地分支（即远端已合并、只是分支被删掉）——**有未提交内容 / 未合入远端的提交一律只报跳过**。
...
| 本地分支越攒越多（MR 合并后远端分支已删） | 点「清理失效分支」：只删提交已合入远端分支的本地分支，有未提交内容/未合并的会列出原因并跳过。 |
