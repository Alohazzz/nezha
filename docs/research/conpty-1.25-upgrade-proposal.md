# 提案：将侧载 ConPTY 从 1.24 升级到 1.25（同步 vendored conpty.dll + OpenConsole.exe）

> Issue 标题（用于 GitHub）：`upgrade(terminal): bump vendored ConPTY from 1.24 to 1.25`
...
理由：**当前侧载版本 1.24.260512001 相对微软最新 release 已落后**。微软在 1.25 及之后持续修复 ConPTY 的输入吞吐与 OpenConsole host 行为；while 现有 1.24 已可用，跟进到 1.25 能吸收这些修复，避免把已知修复留在旧版上。

关于版本选择的取舍（见下方 Scope 末尾），需要 maintainer 拍板是否接受预览版，因此将 **ConPTY 版本目标** 作为本提案的可选项，不预设。
...
- **是否触及[终端性能红线](红线章节)**：
  - 涉及红线中「PTY 输出必须走 `Channel`」「读缓冲 32KB」「批量 flush」「resize 三层防御」「textarea 抑制属性」等 —— **本提案不改动这些常量的值**，只替换 ConPTY 二进制本身，因此理论上不改变这些被实证过的行为。
  - **但**：换版本后的需实测确认性能红线不回归（见下方验证计划）。既然触碰了「终端性能红线」章节的讨论范围，按红线要求，**任何「体感提升」都必须用 `sample` A/B 对照数据印证**，不能只写「测试通过」。
...
microsoft/terminal 当前可用版本（经由 GitHub release API 核实）：
...
2. **resize 稳定性**：TUI 运行中反复拖动窗口改变尺寸，退出后确认 buffer 未被打散（对应 GitHub #4389 的 resize 失同步问题）。
