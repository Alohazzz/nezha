...
1. **看全貌**：按 `workitemId` 实时查询云效议题详情
   （编号 / 标题 / 状态 / 优先级 / 负责人 / 创建时间 / 描述全文），页头附「在云效打开」链接；
   查询失败回退显示本地导入的 `task.prompt`。
...
1. **详情查询**：新增后端命令按 `workitemId` 拉取议题详情；前端详情页展示 +
   「在云效打开」外部链接；失败回退本地 prompt。
...
详情加载中 / 查询失败回退本地 / 表单空态 / AI 预填后 / 人工编辑中 / 定稿后 /
...
2. **详情数据**：按 `workitemId` 实时查询（GetWorkitem），展示编号/标题/状态/优先级/
   负责人/创建时间/描述全文；失败回退本地 prompt；评论/附件不做。
