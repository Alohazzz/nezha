...
创建 MR 有两条路径：**计划链路**（6.1–6.2，先有计划）与「**发起合并**」视图（6.3，直接为**无计划记录**的分支发起，并顺带清理已收尾的远端分支）。

### 6.1 创建计划

欢迎页左侧「**计划**」视图（与云效议题同级）或云效议题视图的「**创建计划**」按钮，都能打开「**创建计划**」对话框：

![创建计划对话框](screenshots/module-guide/02-create-pr.png)
...
计划详情里可查看每个议题的**状态**（未开始 / 讨论中 / 讨论完成 / 执行中 / 已完成 / 已结束）与**关联方案**（全自动派生）；从计划内议题创建的任务**自动落在计划分支 / worktree** 上。详情动作行可「**添加议题**」「**提交 MR**」「**打开**」「**删除计划**」。
...
计划链路要求先有计划；但 agent 用 `git checkout -b` 自建、或你自己推送的分支没有计划记录，走不了 6.2（「发起合并」候选列表会排除已被计划认领的分支）。这类分支走欢迎页左侧的「**发起合并**」视图——它把本地分支元数据与云效平台数据 join 起来，一次看清「哪些分支还没发起 MR、该合到哪、是否已可收尾」。
...
| 创建计划报「主工作区存在未提交改动」 | 未勾选「另建 worktree」时批分支直接切在主工作区，需先提交或暂存当前改动；或改为勾选「另建 worktree」用独立代码目录。 |
