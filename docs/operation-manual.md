...
创建 MR 有两条路径：**批次链路**（6.1–6.2，先有批次/worktree）与「**发起合并**」视图（6.3，直接为**已存在**的分支发起，并顺带清理已收尾的远端分支）。
...
批次链路要求先有「批次 + worktree」；但 agent 用 `git checkout -b` 自建、或你自己推送的分支没有批次记录，走不了 6.2。这类分支走欢迎页左侧的「**发起合并**」视图——它把本地分支元数据与云效平台数据 join 起来，一次看清「哪些分支还没发起 MR、该合到哪、是否已可收尾」。
